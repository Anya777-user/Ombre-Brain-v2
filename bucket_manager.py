...
class BucketManager:
    ...
    def _load_bucket(self, file_path: str) -> Optional[dict]:
        ...
        try:
            ...
            logger.debug("DIAG _load_bucket RAW read_text repr: %s", repr(raw[:300]))
            post_loads = frontmatter.loads(raw)
            logger.debug("DIAG _load_bucket LOADS content repr: %s", repr(str(post_loads.content)[:300]))
            post = frontmatter.load(file_path)
            logger.debug("DIAG _load_bucket LOAD content repr: %s", repr(str(post.content)[:300]))
            return {
